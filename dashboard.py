...
if 'messages' not in st.session_state: 
    st.session_state.messages = [{"role": "assistant", "content": "SYSTEM ONLINE. Ready for audit..."}]
...
def main_dashboard():
    ...
    with st.sidebar:
        ...
        st.radio("Language", ["Tiếng Việt 🇻🇳", "English 🇬🇧"], key="lang_sel", 
                 on_change=update_ui, index=0 if st.session_state.lang=='vi' else 1)
        st.radio("Theme", ["Dark (Cyber) ⚡", "Light (Clean) ☀️"], key="theme_sel", 
                 on_change=update_ui, index=0 if st.session_state.theme=='dark' else 1)
    ...
    # --- B. MAIN UI ---
    if st.session_state.theme == 'dark':
        st.markdown('<h1 style="text-align: center; text-shadow: 0 0 20px #00FFC2;">/// AUDIT CORE V4 ///</h1>', 
                    unsafe_allow_html=True)
    else:
        st.markdown('<h1 style="text-align: center; color: #1e3a8a !important;">✿ FINANCIAL AUDIT HUB ✿</h1>', 
                    unsafe_allow_html=True)
    ...
    with col_R:
        ...
        if st.session_state.error_data:
            with st.expander(f"⚠️ Phát hiện {len(st.session_state.error_data)} dòng lỗi (Bị loại bỏ)"):
                for err in st.session_state.error_data:
                    st.warning(err)
        
        # Xử lý gửi AWS
        if st.session_state.valid_data:
            st.success(f"✅ Đã làm sạch {len(st.session_state.valid_data)} dòng dữ liệu sẵn sàng gửi AI.")
            
            if st.button("🚀 GỬI LÊN AWS", key="send_aws"):
                st.subheader("⚡ PROCESSING LOGS")
                log_container = st.empty()
                logs = []
                bar = st.progress(0)
                
                for i, p in enumerate(st.session_state.valid_data):
                    ts = datetime.now().strftime("%H:%M:%S")
                    bk = p.get('bank_account', 'N/A')
                    logs.append(f"[{ts}] TXN #{i+1} | SUP: {p['supplier']} | BANK: {bk} -> SENT")
                    
                    if st.session_state.theme == 'dark':
                        log_html = "<br>".join([f"<span style='color:#00FFC2; font-family:monospace'>{l}</span>" 
                                               for l in logs[-12:]])
                    else:
                        log_html = "<br>".join([f"<div style='border-bottom:1px solid #eee; padding:4px;'>{l}</div>" 
                                               for l in logs[-12:]])
                    ...
                    bar.progress((i+1)/len(st.session_state.valid_data))
            ...
            with chat_con:
                for msg in st.session_state.messages:
                    ...
                    <div style="text-align: {align}; margin: 10px 0;">
                        <span style="background: {bg}; padding: 10px 15px; border-radius: 15px; 
...
def login_screen():
    if st.session_state.theme == 'dark':
        ...
        st.markdown("""<style>.stApp { background-image: url("https://img.freepik.com/free-vector/white-abstract-background-design_23-2148825582.jpg"); 
    ...
    with c2:
        ...
        st.markdown(f"""
        <div style="background: {box_bg}; padding: 40px; border-radius: 20px; text-align: center; 
                    border: {border}; box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
