...
# Kết nối AWS (Điền Key của bạn vào đây hoặc file secrets.toml)
try:
    sfn = boto3.client(
        'stepfunctions',
        region_name='ap_southeast_1',
        # aws_access_key_id="AKIA_...",    <-- Mở comment và điền key nếu chạy local
...
def inject_css():
    ...
    <style>
        ...
        .cute-card {{ background-color: {card_bg}; padding: 20px; border-radius: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); backdrop-filter: blur(5px); margin-bottom: 20px; }}
        .stTextInput input, .stFileUploader {{ border-radius: 10px; }}
    </style>
    """, unsafe_allow_html=True)

# --- 3. LOGIC ĐỌC EXCEL THẦN THÁNH (DATA INGESTION) ---
def process_and_send(df):
    """Đọc DataFrame -> Chuẩn hóa cột -> Gửi từng dòng sang AWS"""
    # 1. Chuẩn hóa tên cột
    df.columns = df.columns.str.lower().str.strip()
    
    payloads = []
    for _, row in df.iterrows():
        # Logic tìm cột thông minh (Dù file Excel đặt tên cột kiểu gì cũng bắt được)
...
def main_dashboard():
    ...
    with st.sidebar:
        st.title("⚙️ Cài Đặt")
        # Dùng key để quản lý state tự động
        ...
        guide = st.selectbox("Chọn hướng dẫn:", ["-- Chọn --", "Lấy Token Tele", "Lấy Token Zalo"])
    ...
    with col_L:
        ...
        if payloads:
            ...
            for i, p in enumerate(payloads):
                status_txt.code(f"Sending: {p['supplier']} | ${p['invoice_amount']:,.0f}")
            ...
            st.success("✅ HOÀN TẤT! Agent 1 đang xử lý ngầm.")
            st.balloons()
    ...
    with col_R:
        ...
        if prompt := st.chat_input("Hỏi cách dùng app..."):
            ...
            reply = "Bạn cứ up file vào tab bên trái là mình tự xử lý nhé!"
            if "token" in prompt.lower(): reply = "Vào menu bên trái -> Hướng dẫn để xem cách lấy Token nha."
